...
-- 1. Permissões
-- Inserindo os IDs de permissão que serão referenciados pelos perfis
INSERT INTO public.permissoes_usuario (id, super_usuario) VALUES 
(1, true), (2, false), (3, false), (4, false);

-- 2. Perfis (Exatamente como solicitado)
INSERT INTO public.perfil (id, id_permissao, nome) VALUES 
(1, 1, 'Coordenador de Curso'),
(2, 2, 'Tutor'),
(3, 3, 'Coordenador de Ensino'),
(4, 3, 'Coordenador de Unidade'),
(5, 4, 'Público');

-- 3. Instituições (3 reais)
INSERT INTO public.instituicao (cod_mec, nome, tipo) VALUES
(101, 'Universidade de Brasília', 'Pública'),
(102, 'Universidade Federal de Goiás', 'Pública'),
(103, 'Pontifícia Universidade Católica', 'Privada');

-- 4. Unidades (1 por Instituição)
...
-- 5. Cursos
-- Precisamos de 2 cursos na Instituição 101 para que o Tutor seja vinculado a 2 cursos da mesma universidade.
...
-- 6. Usuários (Nomes reais de pessoas, 1 para cada cargo/perfil)
...
-- 7. Usuario_Curso (Apenas Coordenador de Curso e Tutor)
-- Tutor (id 2) ligado a 2 cursos da sua universidade (101)
-- Coord. de Curso (id 1) ligado a 1 curso da sua universidade (102)
INSERT INTO public.usuario_curso (id_usuario, id_curso) VALUES
(2, 1), 
(2, 2), 
(1, 3);

-- 8. Disciplinas (5 por unidade. Como a Unidade 1 tem 2 cursos, dividimos as 5 disciplinas entre eles)
...
-- 9. Turmas (1 por disciplina = 15 turmas)
...
-- 10. Estudantes (75 alunos no total)
-- Distribuição do id_curso reflete a disciplina em que estão matriculados
...
-- 11. Estudante_Turma (Exatos 5 alunos diferentes para cada turma)
...
-- 12. Estatísticas de Desempenho (Risco matematicamente inverso às notas/CH e direto às reprovações)
-- Ciclo de 5 padrões: Excelência (0.05), Bom (0.25), Alerta (0.50), Ruim (0.75), Crítico (0.95)
...
-- 15. Intervenções (Respeitando estritamente os campos restritos do seu CHECK/texto)
-- formato: 'individual' ou 'grupo'
-- interacao: 'pró-ativa' ou 'reativa'
-- tipo: 'Conteudo', 'Acolhimento', 'Outro'
-- acompanhamento: 'Assincrono', 'Sincrono'
-- objetivo_alcancado: 'Sim', 'Nao', 'Parcialmente'
...
-- 16. Estudante_Intervencao
INSERT INTO public.estudante_intervencao (id_intervencao, id_estudante) VALUES
(1, 30), -- Intervenção individual
(2, 1), (2, 2), (2, 3), (2, 4), (2, 5), -- Intervenção em grupo da turma 1
(3, 50), -- Intervenção individual de acolhimento
(4, 21), (4, 22), (4, 23), (4, 24), (4, 25), -- Intervenção em grupo do curso 2
(5, 55); -- Intervenção individual da PUC
